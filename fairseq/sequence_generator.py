        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            with torch.autograd.profiler.record_function(
                ...
                if "image" in net_input and step == 0:
                    ...
                    # first_img_tokens = img_tokens.unsqueeze(1).repeat(1, beam_size, 1).view(bsz * beam_size, -1)
                    ...
                    # modified. June 15, 24
                    decoder_out = self.model.models[0].decoder.forward(
                        first_src_tokens,
                        patch_embeddings=img_tokens,
                        img_gpt_input_mask=first_gpt_input_mask,
                        incremental_state=incremental_states[0],
                        infer_first_step=True)
                    
                    attn: Optional[Tensor] = None
                    decoder_out_tuple = decoder_out[0].div_(self.temperature)
                    decoder_out_tuple = (decoder_out_tuple, None)

                    probs = self.model.models[0].get_normalized_probs(
